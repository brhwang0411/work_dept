...
def get_bm_weight(dt_when):
    ...
    for i in dt_list:
        if dt_when >= i:
            bm_weight = pd.read_sql_query("""SELECT * FROM bm_weight WHERE date='{}'""".format(i), conn,
                                          index_col='date').values.ravel().tolist()
            # ms_norm = [bm_weight[-1]] + [bm_weight[10] - bm_weight[3]] + bm_weight[5:8] + [
            #     bm_weight[8] - bm_weight[-1]] + [bm_weight[9]] + [bm_weight[3]] + [
            #               bm_weight[11] + bm_weight[13] / 4] + [bm_weight[12] + bm_weight[13] / 4]
            # exclude KOR form bm weight first (give KOR portion latter)
            ms_norm = [bm_weight[10] - bm_weight[3]] + bm_weight[5:10] + [bm_weight[3]] + [bm_weight[11] + bm_weight[13] / 4] + [bm_weight[12] + bm_weight[13] / 4]
            ms_norm = ms_norm + [(100 - sum(ms_norm[7:9])) / 2] * 2
            ms_roff = ms_norm[:5] + [0.0] + [ms_norm[6]] + [0.0, 0.0] + [50 + sum(bm_weight[11:13])] + [50 - sum(bm_weight[11:13])]
...
def get_weight(table_name, date):
    with sqlite3.connect(dbpath) as conn:
        weight = pd.read_sql_query('SELECT * FROM "{}" WHERE date="{}"'.format(table_name, date), conn,
                                   index_col='date')
    return weight
...
ms_norm = pd.Series(
    data=bm_weight[0],
    index=['미국주식', '유럽주식', '일본주식', '중국주식', '신흥국주식', '원자재', '글로벌리츠', '하이일드채권', '신흥국채권', '선진국채권', '국내채권']
)
...
up_vol = np.sqrt((np.sum((np.ceil(np.maximum(df_input_ret - avg_ret, 0)) * df_input_ret - up_avg) ** 2) - (
...
down_vol = np.sqrt((np.sum((np.ceil(np.maximum(avg_ret - df_input_ret, 0)) * df_input_ret - down_avg) ** 2) - (
...
sd_adj_delta = ((ewma_sd - hist_sd).where(ewma_sd - hist_sd >= 0) * down_vol / up_vol).replace(np.nan, 0) + (
            (ewma_sd - hist_sd).where(ewma_sd - hist_sd < 0) * up_vol / down_vol).replace(np.nan, 0)
...
if dt_when < '2020-06-07':  # before
    ...
    bgt_score = {
        '80': 1 / adj_sd * array_bgt if sumpos == 0 else mu_hat ** erb_profile['80'][0] / adj_sd ** erb_profile['80'][
            1] * array_bgt,
        '65': 1 / adj_sd * array_bgt if sumpos == 0 else mu_hat ** erb_profile['65'][0] / adj_sd ** erb_profile['65'][
            1] * array_bgt,
        '50': 1 / adj_sd * array_bgt if sumpos == 0 else mu_hat ** erb_profile['50'][0] / adj_sd ** erb_profile['50'][
            1] * array_bgt,
        '35': 1 / adj_sd * array_bgt if sumpos == 0 else mu_hat ** erb_profile['35'][0] / adj_sd ** erb_profile['35'][
            1] * array_bgt,
        '20': 1 / adj_sd * array_bgt if sumpos == 0 else mu_hat ** erb_profile['20'][0] / adj_sd ** erb_profile['20'][
            1] * array_bgt,
    ...
    weight_policy = {
        '80': pd.concat([mksize[:-4] / sum(mksize[:-4]) * erb_profile2['80'][3] * 100,
                         mksize[-4:] / sum(mksize[-4:]) * (1 - erb_profile2['80'][3]) * 100]),
        '65': pd.concat([mksize[:-4] / sum(mksize[:-4]) * erb_profile2['65'][3] * 100,
                         mksize[-4:] / sum(mksize[-4:]) * (1 - erb_profile2['65'][3]) * 100]),
        '50': pd.concat([mksize[:-4] / sum(mksize[:-4]) * erb_profile2['50'][3] * 100,
                         mksize[-4:] / sum(mksize[-4:]) * (1 - erb_profile2['50'][3]) * 100]),
        '35': pd.concat([mksize[:-4] / sum(mksize[:-4]) * erb_profile2['35'][3] * 100,
                         mksize[-4:] / sum(mksize[-4:]) * (1 - erb_profile2['35'][3]) * 100]),
        '20': pd.concat([mksize[:-4] / sum(mksize[:-4]) * erb_profile2['20'][3] * 100,
                         mksize[-4:] / sum(mksize[-4:]) * (1 - erb_profile2['20'][3]) * 100]),
        '10': pd.concat([mksize[:-4] / sum(mksize[:-4]) * erb_profile2['10'][3] * 100,
                         mksize[-4:] / sum(mksize[-4:]) * (1 - erb_profile2['10'][3]) * 100])
    }
    ...
    bgt_score = {
        '80': 1 / adj_sd * array_bgt if sumpos == 0 else mu_hat ** erb_profile2['80'][0] / adj_sd ** erb_profile2['80'][
            1] * array_bgt,
        '65': 1 / adj_sd * array_bgt if sumpos == 0 else mu_hat ** erb_profile2['65'][0] / adj_sd ** erb_profile2['65'][
            1] * array_bgt,
        '50': 1 / adj_sd * array_bgt if sumpos == 0 else mu_hat ** erb_profile2['50'][0] / adj_sd ** erb_profile2['50'][
            1] * array_bgt,
        '35': 1 / adj_sd * array_bgt if sumpos == 0 else mu_hat ** erb_profile2['35'][0] / adj_sd ** erb_profile2['35'][
            1] * array_bgt,
        '20': 1 / adj_sd * array_bgt if sumpos == 0 else mu_hat ** erb_profile2['20'][0] / adj_sd ** erb_profile2['20'][
            1] * array_bgt,
...
if objective_rb(erb['80'], args=(adj_cov2['80'], rc_target['80'])) < 1e-11:
    ...
    if objective_rb(erb['80'], args=(adj_cov2['80'], rc_target['80'])) < 1e-11:
        ...
        if objective_rb(erb['80'], args=(adj_cov2['80'], rc_target['80'])) < 1e-11:
            ...
            erb['80'] = rbweight(adj_cov2['80'], rc_target['80'], get_weight('erb80_hist', df_weekly.index[
                np.where(df_weekly.index == dt_when)[0][0] - 1]).values.ravel()[:-1])
            if objective_rb(erb['80'], args=(adj_cov2['80'], rc_target['80'])) < 1e-11:
                print('Python: ERB_80 Portfolio has been validated')
            else:
...
if objective_rb(erb['65'], args=(adj_cov2['65'], rc_target['65'])) < 1e-11:
    ...
    if objective_rb(erb['65'], args=(adj_cov2['65'], rc_target['65'])) < 1e-11:
        ...
        if objective_rb(erb['65'], args=(adj_cov2['65'], rc_target['65'])) < 1e-11:
            ...
            erb['65'] = rbweight(adj_cov2['65'], rc_target['65'], get_weight('erb65_hist', df_weekly.index[
                np.where(df_weekly.index == dt_when)[0][0] - 1]).values.ravel()[:-1])
            if objective_rb(erb['65'], args=(adj_cov2['65'], rc_target['65'])) < 1e-11:
                print('Python: ERB_65 Portfolio has been validated')
            else:
...
if objective_rb(erb['50'], args=(adj_cov2['50'], rc_target['50'])) < 1e-11:
    ...
    if objective_rb(erb['50'], args=(adj_cov2['50'], rc_target['50'])) < 1e-11:
        ...
        if objective_rb(erb['50'], args=(adj_cov2['50'], rc_target['50'])) < 1e-11:
            ...
            erb['50'] = rbweight(adj_cov2['50'], rc_target['50'], get_weight('erb50_hist', df_weekly.index[
                np.where(df_weekly.index == dt_when)[0][0] - 1]).values.ravel()[:-1])
            if objective_rb(erb['50'], args=(adj_cov2['50'], rc_target['50'])) < 1e-11:
                print('Python: ERB_50 Portfolio has been validated')
            else:
...
if objective_rb(erb['35'], args=(adj_cov2['35'], rc_target['35'])) < 1e-11:
    ...
    if objective_rb(erb['35'], args=(adj_cov2['35'], rc_target['35'])) < 1e-11:
        ...
        if objective_rb(erb['35'], args=(adj_cov2['35'], rc_target['35'])) < 1e-11:
            ...
            erb['35'] = rbweight(adj_cov2['35'], rc_target['35'], get_weight('erb35_hist', df_weekly.index[
                np.where(df_weekly.index == dt_when)[0][0] - 1]).values.ravel()[:-1])
            if objective_rb(erb['35'], args=(adj_cov2['35'], rc_target['35'])) < 1e-11:
                print('Python: ERB_35 Portfolio has been validated')
            else:
...
if objective_rb(erb['20'], args=(adj_cov2['20'], rc_target['20'])) < 1e-11:
    ...
    if objective_rb(erb['20'], args=(adj_cov2['20'], rc_target['20'])) < 1e-11:
        ...
        if objective_rb(erb['20'], args=(adj_cov2['20'], rc_target['20'])) < 1e-11:
            ...
            erb['20'] = rbweight(adj_cov2['20'], rc_target['20'], get_weight('erb20_hist', df_weekly.index[
                np.where(df_weekly.index == dt_when)[0][0] - 1]).values.ravel()[:-1])
            if objective_rb(erb['20'], args=(adj_cov2['20'], rc_target['20'])) < 1e-11:
                print('Python: ERB_20 Portfolio has been validated')
            else:
...
if objective_rb(erb['10'], args=(adj_cov2['10'], rc_target['10'])) < 1e-11:
    ...
    if objective_rb(erb['10'], args=(adj_cov2['10'], rc_target['10'])) < 1e-11:
        ...
        if objective_rb(erb['10'], args=(adj_cov2['10'], rc_target['10'])) < 1e-11:
            ...
            erb['10'] = rbweight(adj_cov2['10'], rc_target['10'], get_weight('erb10_hist', df_weekly.index[
                np.where(df_weekly.index == dt_when)[0][0] - 1]).values.ravel()[:-1])
            if objective_rb(erb['10'], args=(adj_cov2['10'], rc_target['10'])) < 1e-11:
                print('Python: ERB_10 Portfolio has been validated')
            else:
...
if dt_when < '2020-06-07':
    ...
    erb_aft_mod.loc['신흥국주식'][erb_bef_mod.iloc[3:6].sum() - pd.concat(
        [erb_aft_mod.iloc[3:4], erb_aft_mod.iloc[5:6]]).sum() < assetfloor] = 0.0
    erb_aft_mod.loc['신흥국주식'][erb_bef_mod.iloc[3:6].sum() - pd.concat(
        [erb_aft_mod.iloc[3:4], erb_aft_mod.iloc[5:6]]).sum() >= assetfloor] = erb_bef_mod.iloc[4:7].sum() - pd.concat([erb_aft_mod.iloc[3:4], erb_aft_mod.iloc[5:6]]).sum()
    erb_aft_mod.loc['신흥국주식'][erb_aft_mod.loc['신흥국주식'] >= assetcap] = assetcap

    erb_aft_mod.loc['미국주식'] = erb_bef_mod.iloc[:8].sum() - pd.concat(
        [erb_aft_mod.iloc[:1], erb_aft_mod.iloc[2:8]]).sum()
